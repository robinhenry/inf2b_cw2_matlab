...
for k = 1:C
    % Select training samples from class k and add 1 row in case there is only 1
    Xtrn_k = [Xtrn_bin(Ctrn == k, :); zeros(1,D)];
    % Number of samples of class C
    nbr_samples_C = size(Xtrn_k, 1) - 1;      % '-1' because a row was added before
    % P(D_i = 0|C_k) -> using Laplace's rule to avoid 0's
    trn_zeros(k,:) = (sum(Xtrn_k == 0) -1 + 1) / (nbr_samples_C + C);
